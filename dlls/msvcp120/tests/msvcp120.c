...
static void test_tr2_sys__dir_operation(void)
{
    ...
    while(lstrlenA(longer_path) < MAX_PATH)
        strcat(longer_path, "s");
    ok(lstrlenA(longer_path) == MAX_PATH, "tr2_sys__Open_dir(): expect MAX_PATH, got %d\n", lstrlenA(longer_path));
    ...
    todo_wine ok(GetLastError() == ERROR_INSUFFICIENT_BUFFER,
            "tr2_sys__Open_dir(): expect ERROR_INSUFFICIENT_BUFFER, got %d\n", GetLastError());
...
static void test_tr2_sys__Link(void)
{
    ...
    struct {
        ...
        { "f1", "f1_link", TRUE, ERROR_SUCCESS, FALSE },
        { "f1", "tr2_test_dir\\f1_link", TRUE, ERROR_SUCCESS, FALSE },
        { "tr2_test_dir\\f1_link", "tr2_test_dir\\f1_link_link", TRUE, ERROR_SUCCESS, FALSE },
        { "tr2_test_dir", "dir_link", TRUE, ERROR_ACCESS_DENIED, FALSE },
        { NULL, "NULL_link", TRUE, ERROR_INVALID_PARAMETER, TRUE },
        { "f1", NULL, TRUE, ERROR_INVALID_PARAMETER, TRUE },
        { "not_exist", "not_exist_link", TRUE, ERROR_FILE_NOT_FOUND, FALSE },
        { "f1", "not_exist_dir\\f1_link", TRUE, ERROR_PATH_NOT_FOUND, FALSE }
    };
    ...
    for(i=0; i<sizeof(tests)/sizeof(tests[0]); i++) {
        ...
        if(tests[i].is_todo)
            todo_wine ok(ret == tests[i].last_error, "tr2_sys__Link(): test %d expect: %d, got %d\n",
                    i+1, tests[i].last_error, ret);
        else
...
static void test_tr2_sys__Symlink(void)
{
    ...
    struct {
        ...
        { NULL, "NULL_link", ERROR_INVALID_PARAMETER, TRUE },
        { "f1", NULL, ERROR_INVALID_PARAMETER, TRUE },
    ...
    for(i=0; i<sizeof(tests)/sizeof(tests[0]); i++) {
        ...
        if(tests[i].is_todo)
            todo_wine ok(ret == tests[i].last_error, "tr2_sys__Symlink(): test %d expect: %d, got %d\n", i+1, tests[i].last_error, ret);
        else
